...
<html>
   ...
   function getWeekCoinInfo($date){
	  $.post("/wyu34coin/1/index.php/Home/User/getWeekCoinInfo",
	       { 
	       	 opid:'<?php echo ($opid); ?>',
	         coinDate:$date
	        },
		...
		function getWeekStart($date){
			//alert($date);
			$.post(
				 "/wyu34coin/1/index.php/Home/User/getWeekStartAndEnd",
			     {coinDate:$date},	   	     	 	  
	...
	//获取某一周的金币记录（record表）
		function getWeekCoinRecord($date){
				$.post(
	   	     	 	"/wyu34coin/1/index.php/Home/User/getWeekCoinRecord",
	   	     	 	{
	   	     	 	  coinDate:$date,
	   	     	 	  opid:'<?php echo ($opid); ?>'
	   	     	 	},	   	     	 	 
...
$(function () {
		...
		getWeekCoinRecord('<?php echo ($date); ?>');  //取出信息，显示3个图表
		getWeekStart('<?php echo ($date); ?>');     //取出一周的开始日期和结束日期，并显示
        getWeekCoinInfo('<?php echo ($date); ?>');
    //显示日期
     //显示日期选择
